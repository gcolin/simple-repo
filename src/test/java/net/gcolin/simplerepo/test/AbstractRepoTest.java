...
public abstract class AbstractRepoTest {
    ...
    protected Server createServer(int port, String displayName) throws Exception {
        ...
        app.getSecurityHandler().setLoginService(new HashLoginService("Test realm", "src/test/resources/realm.properties"));
    ...
    protected Object executeOperationJmx(String name, String operation,
            Object[] arguments, String[] signature)
            throws Exception {
        MBeanServer server = ManagementFactory.getPlatformMBeanServer();
        ObjectName oname = server.queryNames(
                new ObjectName(name), null).iterator().next();
        return server.invoke(oname, operation, arguments, signature);
    }

    protected void setAttributeJmx(String name, String attribute,
            Object arguments)
            throws Exception {
        MBeanServer server = ManagementFactory.getPlatformMBeanServer();
        ObjectName oname = server.queryNames(
        ...
        server.setAttribute(oname,
                new Attribute(attribute, arguments));
    }

    protected void addRepository(String contextName, String repositoryName,
    ...
    protected void setArtifactMaxAge(String contextName, String repositoryName,
            long maxAge) throws Exception {
        executeOperationJmx("net.gcolin.simplerepo:ctx=" + contextName + ",type=Repository,name=" + repositoryName,
    ...
    protected int sendContent(String url, String fileContent, String user, String password) throws IOException {
        ...
        try {
            ...
            if (user != null) {
                c.setRequestProperty("Authorization", "Basic " + Base64.getEncoder().encodeToString((user + ":" + password).getBytes("utf-8")));
            }
            c.connect();
